...
public class CosplayBot extends BaseBot {
    private interface Config extends BaseConfig {
        String root = "https://tinanime.com/api/genres/cosplay/news/?p=%s";
    }
...
//                    for (int i = 0; i < news.lastPage; i++) {
    ...
    void updateDatabase() {
        try {
            FileUtils.write(new File("out/cosplay.json"), gson.toJson(data), "utf-8");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
